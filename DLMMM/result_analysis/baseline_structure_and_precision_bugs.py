...
if __name__ == '__main__':
    ...
    # TODO 修改为用于保存结果的文件目录
    result_csv = './Ramos_graph_and_precision_bugs.csv'
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        reader = csv.reader(file)
        diff_tf_torch_max = [row[1] for row in reader]
    del diff_tf_torch_max[0]

    with open(file_path, 'r', encoding='utf-8') as file:
        reader = csv.reader(file)
        diff_torch_mindspore_max = [row[3] for row in reader]
    del diff_torch_mindspore_max[0]

    with open(file_path, 'r', encoding='utf-8') as file:
        reader = csv.reader(file)
        diff_tf_mindspore_max = [row[5] for row in reader]
    ...
    for model_structure in model_structrues:
        ...
        #投票机制判断bug来源。
        if diff_tf_torch_max[Round] >= diff_torch_mindspore_max[Round] and diff_tf_mindspore_max[Round] >= diff_torch_mindspore_max[Round]:
            bug_framework = "tensorflow"
            tensorflow_bug_num += 1
        elif diff_torch_mindspore_max[Round] >= diff_tf_mindspore_max[Round] and diff_tf_torch_max[Round] >= diff_tf_mindspore_max[Round]:
            bug_framework = "torch"
            pytorch_bug_num += 1
        else:
